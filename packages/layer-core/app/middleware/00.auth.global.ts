...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  const { fetchSession, loggedIn, user } = useAuth()
  const redirectOptions = useRuntimeConfig().public.auth
  const { only, redirectGuestTo, redirectUserTo } = defu(to.meta?.auth, redirectOptions)
  ...
  if ((routeName as string)?.startsWith('admin') && user.value?.role !== 'admin') {
